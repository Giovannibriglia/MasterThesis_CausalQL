...
class GUI:
    ...
    def __init__(self, rows, cols, n_agents, n_enemies, ag_coord, en_coord, walls_coord, goals_coord, time):
        ...
        self.delay_visualization = 500

        self.font_size = 20
        self.FONT = pygame.font.SysFont('comicsans', self.font_size)

        fix_size = 750
        WIDTH, HEIGHT = fix_size - self.font_size*4, fix_size - self.font_size*4
        self.WINDOW = pygame.display.set_mode((fix_size, fix_size))
...
class CustomEnv:
    ...
    def __init__(self, rows, cols, n_agents, n_act_agents, n_enemies, n_act_enemies, n_goals, if_maze, if_same_enemies_actions, reward_loser):
        self.rows = rows
        self.cols = cols
        self.n_agents = n_agents
        ...
        # positioning enemies
        self.grid_for_game[1][1] = 'En' + str(1)
        self.grid_for_game[1][3] = 'En' + str(2)
        self.grid_for_game[3][0] = 'En' + str(3)
        self.grid_for_game[2][3] = 'En' + str(4)
        pos_enemies = [[1, 1], [1, 3], [3, 0], [2, 3]]
        self.pos_enemies.append([[1, 1], [1, 3], [3, 0], [2, 3]])
        self.pos_enemies_for_reset.append([1, 1])
        self.pos_enemies_for_reset.append([1, 3])
        self.pos_enemies_for_reset.append([3, 0])
        self.pos_enemies_for_reset.append([2, 3])
        # positioning agents
        # positioning agents
        self.pos_agents.append([[0, 0]])
        self.pos_agents_for_reset.append([0, 0])
        self.grid_for_game[0][0] = 'Ag' + str(1)
        ...
        # positioning goal
        self.grid_for_game[3][3] = 'Goal' + str(1)
        self.pos_goals.append([3, 3])
...
def plot_RLmetrics(rows, cols, vet_average_episodes_rewards_allGames, vet_steps_for_episode_allGames, dir,
    ...
    fig.suptitle(f'{word_kindGame} {rows}x{cols} - {n_games} games averaged - Reward defeat: {env.reward_loser}')
...
def plot_Time(rows, cols, vet_time_for_AllGames, dir, labels_for_plot, vet_timeouts_algs):
    ...
    plt.title(f'{word_kindGame} {rows}x{cols} - {n_games} games averaged - Reward defeat: {env.reward_loser}')
    ...
    plt.savefig(dir + f'\\{word_kindGame}_{rows}x{cols}_Times.png')
    plt.show()
...
def plot_n_times_loser(rows, cols, vet_losers_for_AllGames, dir, labels_for_plot, vet_timeouts_algs):
    ...
    plt.grid()
    plt.subplots_adjust(bottom=0.17, left=0.12)
    plt.savefig(dir + f'\\{word_kindGame}_{rows}x{cols}_Losers.png')
    plt.show()
...
def plot_n_timouts(rows, cols, vet_timeouts_algs, dir, labels_for_plot, indexes_ok):
    ...
    plt.grid()
    plt.subplots_adjust(bottom=0.17, left=0.12)
    plt.savefig(dir + f'\\{word_kindGame}_{rows}x{cols}_Timeouts.png')
    plt.show()
...
for maze in [True]:
    for same_enemies_actions in [True]:
        ...
        # modify folder
        word_kindGame = 'FrozenLake'
        if_maze = False
        if_same_enemies_actions = False

        # folder
        dir_principle = f"{word_kindGame}_RewardLoser{reward_loser}_{len(approaches_choices)}alg_Averaged{n_games}Games"

        for n_agents in [1]:
            for n_enemies in [4]:
                for n_act_agents in [5]:
                    for n_act_enemies in [1]:

                        # folder for saving plots
                        if not os.path.exists(dir_principle):
                            os.mkdir(dir_principle)
                        dir_secondary = dir_principle
                            ...
                            if n_enemies > rows * 2:
                                ...
                                for game in range(n_games):
                                    env = CustomEnv(rows=rows, cols=cols, n_agents=n_agents, n_act_agents=n_act_agents, n_enemies=n_enemies,
                                                n_act_enemies=n_act_enemies, n_goals=n_goals, if_maze=if_maze, if_same_enemies_actions=if_same_enemies_actions, reward_loser=reward_loser)
