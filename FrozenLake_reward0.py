...
def plot_RLmetrics(rows, cols, vet_average_episodes_rewards_allGames, vet_steps_for_episode_allGames, dir, table_for_saving, labels_for_plot):
    ...
    fig.suptitle(f'{word_kindGame} {rows}x{cols} - {n_games} games averaged - Reward loser = 0')

    results_for_saving = []
    for select_alg in range(n_algs_plot):

        average_reward_to_saving = round(np.mean(av_rew_final[select_alg]), 5)
        ...
        ax1.set_title('Average reward divided by steps for episode')
...
def plot_Time(rows, cols, vet_time_for_AllGames, dir, labels_for_plot):
    width = 0.5
    fontsize_ticks = 8
    fontsize_labels = 12
    ...
    plt.title(f'{word_kindGame} {rows}x{cols} - {n_games} games averaged - Reward loser = 0')
    plt.xticks(fontsize=fontsize_ticks, rotation=90)
    plt.yticks(fontsize=fontsize_ticks)
    plt.xlabel('Approach used', fontsize=fontsize_labels)
    plt.ylabel('Average time neeeded [min]', fontsize=fontsize_labels, labelpad=5)
    plt.grid()
...
def plot_n_times_loser(rows, cols, vet_losers_for_AllGames, dir, labels_for_plot):
    width = 0.5
    fontsize_ticks = 8
    fontsize_labels = 12
    ...
    plt.yticks(fontsize=fontsize_ticks)
    plt.xlabel('Approach used', fontsize=fontsize_labels)
    plt.ylabel('Average loser times', fontsize=fontsize_labels, labelpad=5)
    plt.grid()
    plt.subplots_adjust(bottom=0.17, left=0.12)
...
for maze in [False]:
    for same_enemies_actions in [True]:
        ...
        # folder
        dir_principle = f"ADD_{word_kindGame}_{len(approaches_choices)}alg_Averaged{n_games}Games_{word_EnemiesAct}_simpleEnv"
